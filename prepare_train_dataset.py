...
def get_arguments():
    ...
    parser.add_argument('--video_folder', default='/gpu02/dataset_public/AI+4K_HDR/train/',
                        help='folder with video')
    parser.add_argument('--dataset_folder', default='./train',
...
def process_video(lr_filepath,args):
    hr_filepath=lr_filepath.replace('SDR_540p', 'SDR_4K')
    ...
    hr_seq_save_path=os.path.join(video_savepath,'hr')
    if not os.path.exists(lr_seq_save_path):
        os.mkdir(lr_seq_save_path)
    if not os.path.exists(hr_seq_save_path):
        os.mkdir(hr_seq_save_path)
    f = open('train_lr.txt', 'a+')
    cap_lr = cv2.VideoCapture(lr_filepath)
    cap_hr = cv2.VideoCapture(hr_filepath)
    frame_count=0
    success=True
    while(success):
        success_lr,frame_lr=cap_lr.read()
        success_hr,frame_hr=cap_hr.read()
        success=success_lr and success_hr
        if success:
            cv2.imwrite(os.path.join(lr_seq_save_path ,"%03d.png" % frame_count), frame_lr,[cv2.IMWRITE_PNG_COMPRESSION,0])
            cv2.imwrite(os.path.join(hr_seq_save_path, "%03d.png" % frame_count), frame_hr,[cv2.IMWRITE_PNG_COMPRESSION,0])
