...
def load_img_future(filepath, nFrames, scale, other_dataset):
    ...
    if other_dataset:
        ...
        hr_filepath=filepath.replace('lr','hr')
...
def load_img_future_test(filepath, nFrames, scale, other_dataset):
    tt = int(nFrames/2)
    if other_dataset:
        input = modcrop(Image.open(filepath).convert('RGB'),scale)
...
class DatasetFromFolder(data.Dataset):
    def __init__(self, image_dir,nFrames, upscale_factor, data_augmentation, file_list, other_dataset, patch_size, future_frame, transform=None):
        ...
        self.image_filenames = [x for x in alist]
...
def chop_get_flow(input, neigbor, shave=8, min_size=2000):
    h,w,_= np.shape(input)
    h_half, w_half = h // 2, w // 2
    h_size, w_size = h_half + shave, w_half + shave
    inputlist = [
        [input[0:h_size, 0:w_size,:], neigbor[ 0:h_size, 0:w_size,:]],
        [input[0:h_size, (w - w_size):w,:], neigbor[0:h_size, (w - w_size):w,:]],
        [input[(h - h_size):h, 0:w_size,:], neigbor[(h - h_size):h, 0:w_size,:]],
        [input[(h - h_size):h, (w - w_size):w,:], neigbor[(h - h_size):h, (w - w_size):w,:]]]
    ...
    if w_size * h_size < min_size:
        ...
        for i in range(0, 4):
            ...
            output_batch = np.array(get_flow(Image.fromarray(input_batch[0]),Image.fromarray(input_batch[1])))
            outputlist.append(output_batch)
    else:
        outputlist = [
            chop_get_flow(patch[0], patch[1],shave, min_size) \
...
class DatasetFromFolderTest(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.future_frame:
            input, neigbor = load_img_future_test(self.image_filenames[index], self.nFrames, self.upscale_factor, self.other_dataset)
        else:
            target, input, neigbor = load_img(self.image_filenames[index], self.nFrames, self.upscale_factor, self.other_dataset)

            
        #flow = [chop_get_flow(np.array(input),np.array(j)) for j in neigbor]
        flow = [get_flow(input, j) for j in neigbor]
        ...
        if self.transform:
            ...
            neigbor = [self.transform(j) for j in neigbor]
            flow = [torch.from_numpy(j.transpose(2,0,1)) for j in flow]
        video_name,_=os.path.split(self.image_filenames[index])
        video_name,_=os.path.split(video_name)
        _,video_name= os.path.split(video_name)
            
        return input, neigbor, flow, bicubic,video_name
